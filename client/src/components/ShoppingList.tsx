...
import { ShoppingListItem, groupShoppingListByCategory, exportShoppingListToText } from "@/utils/shopping";
...
export function ShoppingList({ items, recipeName }: ShoppingListProps) {
  ...
  const handleExport = () => {
    ...
    a.href = url;
    a.download = `${recipeName.replace(/[^a-z0-9]/gi, "-").toLowerCase()}-shopping-list.txt`;
    a.click();
